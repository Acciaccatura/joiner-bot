...
var siftChannels = function(slapp, token, query, callback) {
	slapp.client.channels.list({'token': token}, (err, data) => {
		if (err || !query) { 
			...
			let fuse = new Fuse(data.channels, options)
...
module.exports = {
	...
	test: (slapp, callback) => {
		slapp.route('someValue', (msg, state) => {
			console.log('SUCCESS:', msg.body.actions, msg.body.actions[0])
			msg.respond('Got them!').route('someValue2', null)
		})
		slapp.route('someValue2', (msg, state) => {
			msg.
		})
	}
